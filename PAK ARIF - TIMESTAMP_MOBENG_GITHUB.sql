...
--INSERT
--SELECT * FROM MB_DB_Reporting_SBY.dbo.MB_Transaksi_Timestamp;
TRUNCATE TABLE MB_DB_Reporting_SBY.dbo.MB_Transaksi_Timestamp;
...
SELECT * FROM MB_DB_Reporting_SBY.dbo.MB_Trx_Timestamp(@date1, @date2);
...
--------------------------------------------------------------------------------------------------------------------------SQL SERVER RMS10/JAKARTA


--RMS10
--FUNCTION RMB
--select * from [MB_DB_Reporting_JKT].[dbo].[MB_Transaksi_RMB] ('2024-01-01','2024-02-29') where rmb=1;
    ...
    (
        ...
        (
            SELECT  
                ...
                END AS tbic
            FROM 
                MB_DB_Reporting_JKT.dbo.mb_rms20_transaksi_toko_perjenis_member_v3 AS x1
            WHERE 
                x1.tanggal BETWEEN @date1 AND @date2
                AND x1.statusproduk <> 'K'
                AND x1.namacabang = 'Surabaya'
...
--RMS10
--TABLE TEMP
--DROP TABLE MB_DB_Reporting_JKT.dbo.MB_Transaksi_Timestamp;
--SELECT * FROM MB_DB_Reporting_JKT.dbo.MB_Transaksi_Timestamp;

CREATE TABLE MB_DB_Reporting_JKT.dbo.MB_Transaksi_Timestamp (
insert_date datetime NOT NULL,
transaction_date date NOT NULL,
storecode BIGINT NOT NULL,
storename VARCHAR(100) NOT NULL,
...
(
    SELECT
        ...
        g.trans_status_date AS POS_Start_Transaction_time,
        a.tglbayar AS Print_Struk
    FROM mb_dc.dbo.transaksitokoheader AS a WITH (NOLOCK)
    JOIN mb_dc.dbo.smitransaksitokoperjenismember AS b WITH (NOLOCK)
        ON b.kodetoko = a.kodetoko AND b.NomorTransaksi = a.nomorTransaksi
    JOIN mb_dc.dbo.msttoko AS d WITH (NOLOCK)
        ON d.kodetoko = a.kodetoko
    LEFT JOIN MB_DB_Reporting_JKT.dbo.mb_transaksi_rmb(@date1, @date2) AS e
        ON e.kodetoko = a.kodetoko AND e.nomortransaksi = a.nomorTransaksi
    LEFT JOIN 
    (
        SELECT 
            *,
            ROW_NUMBER() OVER (PARTITION BY kode_toko, trans_id ORDER BY trans_date DESC) AS myrank
        FROM mb_dc.dbo.imen_trans_header WITH (NOLOCK)
        WHERE 
            CONVERT(DATE, trans_date) BETWEEN @date1 AND @date2
            AND trans_status = 1000000004
    ) AS f
    ...
    (
        SELECT 
            *,
            ROW_NUMBER() OVER (PARTITION BY kode_toko, trans_id ORDER BY trans_status_date DESC) AS myrank
        FROM mb_dc.dbo.Imen_Trans_Status_Hist WITH (NOLOCK)
        WHERE 
            CONVERT(DATE, trans_status_date) BETWEEN @date1 AND @date2
            AND trans_status = 1000000004
    ) AS g
...
--INSERT
--SELECT * FROM MB_DB_Reporting_JKT.dbo.MB_Transaksi_Timestamp;
TRUNCATE TABLE MB_DB_Reporting_JKT.dbo.MB_Transaksi_Timestamp;

DECLARE @date1 DATE=convert(date,DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-1, 0)) -- periode awal data M-1
DECLARE @date2 DATE=convert(date,dateadd(d,-(day(getdate())),getdate()))	-- periode akhir data M-1

INSERT INTO MB_DB_Reporting_JKT.dbo.MB_Transaksi_Timestamp
SELECT * FROM MB_DB_Reporting_JKT.dbo.MB_Trx_Timestamp(@date1, @date2);
...
-- MB_rms10_RPT
-- FOREIGN TABLE
-- SELECT * FROM mb_rms10_rpt.mb_transaksi_timestamp definition
-- DROP FOREIGN TABLE mb_rms10_rpt.mb_transaksi_timestamp;

CREATE FOREIGN TABLE mb_rms10_rpt.mb_transaksi_timestamp (
	insert_date timestamp OPTIONS(column_name 'insert_date') NOT NULL,
	transaction_date date OPTIONS(column_name 'transaction_date') NOT NULL,
	storecode int8 OPTIONS(column_name 'storecode') NOT NULL,
	storename varchar(100) OPTIONS(column_name 'storename') NOT NULL,
